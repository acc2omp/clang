//===----- CGOpenACCRuntime.h - Interface to OpenACC Runtimes -----*- C++ -*-===//
//
...
#ifndef LLVM_CLANG_LIB_CODEGEN_CGOPENACCRUNTIME_H
...
class Expr;
class GlobalDecl;
class ACCDependClause;
class ACCExecutableDirective;
class ACCLoopLikeDirective;
class VarDecl;
...
struct ACCTaskDataTy final {
  ...
  SmallVector<std::pair<OpenACCDependClauseKind, const Expr *>, 4> Dependences;
...
protected:
  ...
  /// evaluates to false.
  virtual void emitTargetOutlinedFunctionHelper(const ACCExecutableDirective &D,
...
private:
  ...
  /// ident_t objects.
  llvm::Constant *DefaultOpenACCPSource = nullptr;
  /// \brief Map of flags and corresponding default locations.
  typedef llvm::DenseMap<unsigned, llvm::Value *> OpenACCDefaultLocMapTy;
  OpenACCDefaultLocMapTy OpenACCDefaultLocMap;
  Address getOrCreateDefaultLocation(unsigned Flags);

  llvm::StructType *IdentTy = nullptr;
  /// \brief Map for SourceLocation and OpenACC runtime library debug locations.
  typedef llvm::DenseMap<unsigned, llvm::Value *> OpenACCDebugLocMapTy;
  OpenACCDebugLocMapTy OpenACCDebugLocMap;
  ...
  typedef llvm::DenseMap<llvm::Function *, DebugLocThreadIdTy>
      OpenACCLocThreadIDMapTy;
  OpenACCLocThreadIDMapTy OpenACCLocThreadIDMap;
  /// Map of UDRs and corresponding combiner/initializer.
  typedef llvm::DenseMap<const ACCDeclareReductionDecl *,
                         std::pair<llvm::Function *, llvm::Function *>>
      UDRMapTy;
  UDRMapTy UDRMap;
  /// Map of functions and locally defined UDRs.
  typedef llvm::DenseMap<llvm::Function *,
                         SmallVector<const ACCDeclareReductionDecl *, 4>>
      FunctionUDRMapTy;
  ...
  /// <critical_section_name> + ".var" for "acc critical" directives; 2)
  ...
  /// \brief Returns specified OpenACC runtime function.
  ...
  /// storage that's reserved for use by the OpenACC runtime.
...
public:
  explicit CGOpenACCRuntime(CodeGenModule &CGM);
  virtual ~CGOpenACCRuntime() {}
  virtual void clear();

  /// Emit code for the specified user defined reduction construct.
  virtual void emitUserDefinedReduction(CodeGenFunction *CGF,
                                        const ACCDeclareReductionDecl *D);
  /// Get combiner/initializer for the specified user-defined reduction, if any.
  virtual std::pair<llvm::Function *, llvm::Function *>
  getUserDefinedReduction(const ACCDeclareReductionDecl *D);

  /// \brief Emits outlined function for the specified OpenACC parallel directive
  /// \a D. This outlined function has type void(*)(kmp_int32 *ThreadID,
  /// kmp_int32 BoundID, struct context_vars*).
  /// \param D OpenACC directive.
  /// \param ThreadIDVar Variable for thread id in the current OpenACC region.
  ...
  /// \param D OpenACC directive.
  /// \param ThreadIDVar Variable for thread id in the current OpenACC region.
  /// \param PartIDVar Variable for partition id in the current OpenACC untied
  /// task region.
  ...
  /// generated. Must be ACCD_barrier for explicit barrier generation.
  ...
  ///
  virtual bool isStaticNonchunked(OpenACCScheduleClauseKind ScheduleKind,
                                  bool Chunked) const;
  ...
  /// \param Chunked True if chunk is specified in the clause.
  ///
  virtual bool isStaticNonchunked(OpenACCDistScheduleClauseKind ScheduleKind,
                                  bool Chunked) const;
  ...
  /// \param ScheduleKind Schedule Kind specified in the 'schedule' clause.
  ///
  virtual bool isDynamic(OpenACCScheduleClauseKind ScheduleKind) const;
  ...
  /// routine before start of the OpenACC loop to get the loop upper / lower
  ...
  /// \brief Emit flush of the variables specified in 'acc flush' directive.
  ...
  struct ReductionOptionsTy {
    ...
    OpenACCDirectiveKind ReductionKind;
  };
  ...
  ///     otherwise.
  ///     SimpleReduction Emit reduction operation only. Used for acc simd
  ...
  /// An outlined function may not be an entry if, e.g. the if clause always
  /// evaluates to false.
  virtual void emitTargetOutlinedFunction(const ACCExecutableDirective &D,
                                          StringRef ParentName,
                                          llvm::Function *&OutlinedFn,
                                          llvm::Constant *&OutlinedFnID,
                                          bool IsOffloadEntry,
...
public:
  ...
  void emitBarrierCall(CodeGenFunction &CGF, SourceLocation Loc,
                       OpenACCDirectiveKind Kind, bool EmitChecks = true,
  ...
  void emitForStaticFinish(CodeGenFunction &CGF, SourceLocation Loc,
                           OpenACCDirectiveKind DKind) override;
  ...
  void emitTaskCall(CodeGenFunction &CGF, SourceLocation Loc,
                    const ACCExecutableDirective &D, llvm::Value *TaskFunction,
                    QualType SharedsTy, Address Shareds, const Expr *IfCond,
                    const ACCTaskDataTy &Data) override;
  ...
  void emitTaskLoopCall(CodeGenFunction &CGF, SourceLocation Loc,
                        const ACCLoopLikeDirective &D, llvm::Value *TaskFunction,
                        QualType SharedsTy, Address Shareds, const Expr *IfCond,
                        const ACCTaskDataTy &Data) override;
  ...
  void emitCancellationPointCall(CodeGenFunction &CGF, SourceLocation Loc,
                                 OpenACCDirectiveKind CancelRegion) override;
  ...
  /// An outlined function may not be an entry if, e.g. the if clause always
  /// evaluates to false.
  void emitTargetOutlinedFunction(const ACCExecutableDirective &D,
                                  StringRef ParentName,
                                  llvm::Function *&OutlinedFn,
                                  llvm::Constant *&OutlinedFnID,
                                  bool IsOffloadEntry,
  ...
  /// \param Device Expression evaluated in device clause associated with the
  /// target directive, or null if no device clause is used.
  void emitTargetCall(CodeGenFunction &CGF, const ACCExecutableDirective &D,
  ...
  void emitDoacrossInit(CodeGenFunction &CGF,
                        const ACCLoopLikeDirective &D) override;

  /// Emit code for doacross ordered directive with 'depend' clause.
  /// \param C 'depend' clause with 'sink|source' dependency kind.
  void emitDoacrossOrdered(CodeGenFunction &CGF,
                           const ACCDependClause *C) override;
