//===---- CGOpenACCRuntimeNVPTX.cpp - Interface to OpenACC NVPTX Runtimes ---===//
//
...
//===----------------------------------------------------------------------===//
//
// This provides a class for OpenACC runtime code generation specialized to NVPTX
// targets.
//
//===----------------------------------------------------------------------===//
...
namespace {
enum OpenACCRTLFunctionNVPTX {
  /// \brief Call to void __kmpc_kernel_init(kmp_int32 thread_limit,
  /// int16_t RequiresACCRuntime);
  ACCRTL_NVPTX__kmpc_kernel_init,
  /// \brief Call to void __kmpc_kernel_deinit(int16_t IsACCRuntimeInitialized);
  ACCRTL_NVPTX__kmpc_kernel_deinit,
  /// \brief Call to void __kmpc_spmd_kernel_init(kmp_int32 thread_limit,
  /// int16_t RequiresACCRuntime, int16_t RequiresDataSharing);
  ACCRTL_NVPTX__kmpc_spmd_kernel_init,
  /// \brief Call to void __kmpc_spmd_kernel_deinit();
  ACCRTL_NVPTX__kmpc_spmd_kernel_deinit,
  /// \brief Call to void __kmpc_kernel_prepare_parallel(void
  /// *outlined_function, void ***args, kmp_int32 nArgs, int16_t
  /// IsACCRuntimeInitialized);
  ACCRTL_NVPTX__kmpc_kernel_prepare_parallel,
  /// \brief Call to bool __kmpc_kernel_parallel(void **outlined_function, void
  /// ***args, int16_t IsACCRuntimeInitialized);
  ACCRTL_NVPTX__kmpc_kernel_parallel,
  /// \brief Call to void __kmpc_kernel_end_parallel();
  ACCRTL_NVPTX__kmpc_kernel_end_parallel,
  /// Call to void __kmpc_serialized_parallel(ident_t *loc, kmp_int32
  /// global_tid);
  ACCRTL_NVPTX__kmpc_serialized_parallel,
  /// Call to void __kmpc_end_serialized_parallel(ident_t *loc, kmp_int32
  /// global_tid);
  ACCRTL_NVPTX__kmpc_end_serialized_parallel,
  /// \brief Call to int32_t __kmpc_shuffle_int32(int32_t element,
  /// int16_t lane_offset, int16_t warp_size);
  ACCRTL_NVPTX__kmpc_shuffle_int32,
  /// \brief Call to int64_t __kmpc_shuffle_int64(int64_t element,
  /// int16_t lane_offset, int16_t warp_size);
  ACCRTL_NVPTX__kmpc_shuffle_int64,
  /// \brief Call to __kmpc_nvptx_parallel_reduce_nowait(kmp_int32
  /// global_tid, kmp_int32 num_vars, size_t reduce_size, void* reduce_data,
  /// void (*kmp_ShuffleReductFctPtr)(void *rhsData, int16_t lane_id, int16_t
  /// lane_offset, int16_t shortCircuit),
  /// void (*kmp_InterWarpCopyFctPtr)(void* src, int32_t warp_num));
  ACCRTL_NVPTX__kmpc_parallel_reduce_nowait,
  ...
  ACCRTL_NVPTX__kmpc_teams_reduce_nowait,
  /// \brief Call to __kmpc_nvptx_end_reduce_nowait(int32_t global_tid);
  ACCRTL_NVPTX__kmpc_end_reduce_nowait
};

/// Pre(post)-action for different OpenACC constructs specialized for NVPTX.
class NVPTXActionTy final : public ACCPrePostActionTy {
...
private:
  CGOpenACCRuntimeNVPTX::ExecutionMode SavedMode;
  CGOpenACCRuntimeNVPTX::ExecutionMode &Mode;

public:
  ExecutionModeRAII(CGOpenACCRuntimeNVPTX::ExecutionMode &Mode,
                    CGOpenACCRuntimeNVPTX::ExecutionMode NewMode)
      : Mode(Mode) {
    SavedMode = Mode;
    Mode = NewMode;
  }
...
/// Get the thread id of the ACC master thread.
...
CGOpenACCRuntimeNVPTX::WorkerFunctionState::WorkerFunctionState(
    CodeGenModule &CGM, SourceLocation Loc)
    : WorkerFn(nullptr), CGFI(nullptr), Loc(Loc) {
  createWorkerFunction(CGM);
}

void CGOpenACCRuntimeNVPTX::WorkerFunctionState::createWorkerFunction(
    CodeGenModule &CGM) {
...
bool CGOpenACCRuntimeNVPTX::isInSpmdExecutionMode() const {
  return CurrentExecutionMode == CGOpenACCRuntimeNVPTX::ExecutionMode::Spmd;
}

static CGOpenACCRuntimeNVPTX::ExecutionMode
getExecutionModeForDirective(CodeGenModule &CGM,
                             const ACCExecutableDirective &D) {
  OpenACCDirectiveKind DirectiveKind = D.getDirectiveKind();
  switch (DirectiveKind) {
  case ACCD_target:
  case ACCD_target_gang:
    return CGOpenACCRuntimeNVPTX::ExecutionMode::Generic;
  case ACCD_target_parallel:
  case ACCD_target_parallel_loop:
  case ACCD_target_parallel_loop_vector:
    return CGOpenACCRuntimeNVPTX::ExecutionMode::Spmd;
  default:
...
void CGOpenACCRuntimeNVPTX::emitGenericKernel(const ACCExecutableDirective &D,
  ...
  ExecutionModeRAII ModeRAII(CurrentExecutionMode,
                             CGOpenACCRuntimeNVPTX::ExecutionMode::Generic);
  ...
  class NVPTXPrePostActionTy : public ACCPrePostActionTy {
    CGOpenACCRuntimeNVPTX::EntryFunctionState &EST;
    CGOpenACCRuntimeNVPTX::WorkerFunctionState &WST;

  public:
    NVPTXPrePostActionTy(CGOpenACCRuntimeNVPTX::EntryFunctionState &EST,
                         CGOpenACCRuntimeNVPTX::WorkerFunctionState &WST)
        : EST(EST), WST(WST) {}
    void Enter(CodeGenFunction &CGF) override {
      static_cast<CGOpenACCRuntimeNVPTX &>(CGF.CGM.getOpenACCRuntime())
...
// Setup NVPTX threads for master-worker OpenACC scheme.
void CGOpenACCRuntimeNVPTX::emitGenericEntryHeader(CodeGenFunction &CGF,
                                                  EntryFunctionState &EST,
  ...
  // Initialize the state of the OpenACC runtime library on the GPU.
  // TODO: Optimize runtime initialization and pass in correct value.
  llvm::Value *Args[] = {getThreadLimit(CGF),
                         Bld.getInt16(/*RequiresACCRuntime=*/1)};
  CGF.EmitRuntimeCall(
      createNVPTXRuntimeFunction(ACCRTL_NVPTX__kmpc_kernel_init), Args);
}

void CGOpenACCRuntimeNVPTX::emitGenericEntryFooter(CodeGenFunction &CGF,
                                                  EntryFunctionState &EST) {
  if (!EST.ExitBB)
    EST.ExitBB = CGF.createBasicBlock(".exit");
  ...
  // TODO: Optimize runtime initialization and pass in correct value.
  llvm::Value *Args[] = {CGF.Builder.getInt16(/*IsACCRuntimeInitialized=*/1)};
  CGF.EmitRuntimeCall(
      createNVPTXRuntimeFunction(ACCRTL_NVPTX__kmpc_kernel_deinit), Args);
...
void CGOpenACCRuntimeNVPTX::emitSpmdKernel(const ACCExecutableDirective &D,
                                          StringRef ParentName,
                                          llvm::Function *&OutlinedFn,
                                          llvm::Constant *&OutlinedFnID,
                                          bool IsOffloadEntry,
                                          const ACCRegionCodeGenTy &CodeGen) {
  ExecutionModeRAII ModeRAII(CurrentExecutionMode,
                             CGOpenACCRuntimeNVPTX::ExecutionMode::Spmd);
  EntryFunctionState EST;

  // Emit target region as a standalone region.
  class NVPTXPrePostActionTy : public ACCPrePostActionTy {
    CGOpenACCRuntimeNVPTX &RT;
    CGOpenACCRuntimeNVPTX::EntryFunctionState &EST;
    const ACCExecutableDirective &D;

  public:
    NVPTXPrePostActionTy(CGOpenACCRuntimeNVPTX &RT,
                         CGOpenACCRuntimeNVPTX::EntryFunctionState &EST,
                         const ACCExecutableDirective &D)
        : RT(RT), EST(EST), D(D) {}
    void Enter(CodeGenFunction &CGF) override {
...
void CGOpenACCRuntimeNVPTX::emitSpmdEntryHeader(
  ...
  // Initialize the ACC state in the runtime; called by all active threads.
  // TODO: Set RequiresACCRuntime and RequiresDataSharing parameters
  // based on code analysis of the target region.
  llvm::Value *Args[] = {getThreadLimit(CGF, /*IsInSpmdExecutionMode=*/true),
                         /*RequiresACCRuntime=*/Bld.getInt16(1),
                         /*RequiresDataSharing=*/Bld.getInt16(1)};
  CGF.EmitRuntimeCall(
      createNVPTXRuntimeFunction(ACCRTL_NVPTX__kmpc_spmd_kernel_init), Args);
  CGF.EmitBranch(ExecuteBB);

  CGF.EmitBlock(ExecuteBB);
}

void CGOpenACCRuntimeNVPTX::emitSpmdEntryFooter(CodeGenFunction &CGF,
                                               EntryFunctionState &EST) {
  if (!EST.ExitBB)
    EST.ExitBB = CGF.createBasicBlock(".exit");

  llvm::BasicBlock *ACCDeInitBB = CGF.createBasicBlock(".omp.deinit");
  CGF.EmitBranch(ACCDeInitBB);

  CGF.EmitBlock(ACCDeInitBB);
  // DeInitialize the ACC state in the runtime; called by all active threads.
  CGF.EmitRuntimeCall(
      createNVPTXRuntimeFunction(ACCRTL_NVPTX__kmpc_spmd_kernel_deinit), None);
  CGF.EmitBranch(EST.ExitBB);
...
void CGOpenACCRuntimeNVPTX::emitWorkerFunction(WorkerFunctionState &WST) {
...
void CGOpenACCRuntimeNVPTX::emitWorkerLoop(CodeGenFunction &CGF,
                                          WorkerFunctionState &WST) {
  //
  ...
  llvm::Value *Ret = CGF.EmitRuntimeCall(
      createNVPTXRuntimeFunction(ACCRTL_NVPTX__kmpc_kernel_parallel), Args);
  ...
  CGF.EmitBlock(TerminateBB);
  CGF.EmitRuntimeCall(
      createNVPTXRuntimeFunction(ACCRTL_NVPTX__kmpc_kernel_end_parallel),
      llvm::None);
...
/// \brief Returns specified OpenACC runtime function for the current OpenACC
/// implementation.  Specialized for the NVPTX device.
/// \param Function OpenACC runtime function.
/// \return Specified function.
llvm::Constant *
CGOpenACCRuntimeNVPTX::createNVPTXRuntimeFunction(unsigned Function) {
  llvm::Constant *RTLFn = nullptr;
  switch (static_cast<OpenACCRTLFunctionNVPTX>(Function)) {
  case ACCRTL_NVPTX__kmpc_kernel_init: {
  ...
  case ACCRTL_NVPTX__kmpc_kernel_deinit: {
    // Build void __kmpc_kernel_deinit(int16_t IsACCRuntimeInitialized);
  ...
  case ACCRTL_NVPTX__kmpc_spmd_kernel_init: {
  ...
  case ACCRTL_NVPTX__kmpc_spmd_kernel_deinit: {
  ...
  case ACCRTL_NVPTX__kmpc_kernel_prepare_parallel: {
  ...
  case ACCRTL_NVPTX__kmpc_kernel_parallel: {
  ...
  case ACCRTL_NVPTX__kmpc_kernel_end_parallel: {
  ...
  case ACCRTL_NVPTX__kmpc_serialized_parallel: {
  ...
  case ACCRTL_NVPTX__kmpc_end_serialized_parallel: {
  ...
  case ACCRTL_NVPTX__kmpc_shuffle_int32: {
  ...
  case ACCRTL_NVPTX__kmpc_shuffle_int64: {
  ...
  case ACCRTL_NVPTX__kmpc_parallel_reduce_nowait: {
  ...
  case ACCRTL_NVPTX__kmpc_teams_reduce_nowait: {
  ...
  case ACCRTL_NVPTX__kmpc_end_reduce_nowait: {
...
void CGOpenACCRuntimeNVPTX::createOffloadEntry(llvm::Constant *ID,
                                              llvm::Constant *Addr,
...
void CGOpenACCRuntimeNVPTX::emitTargetOutlinedFunction(
  ...
  switch (Mode) {
  case CGOpenACCRuntimeNVPTX::ExecutionMode::Generic:
    emitGenericKernel(D, ParentName, OutlinedFn, OutlinedFnID, IsOffloadEntry,
                      CodeGen);
    break;
  case CGOpenACCRuntimeNVPTX::ExecutionMode::Spmd:
    emitSpmdKernel(D, ParentName, OutlinedFn, OutlinedFnID, IsOffloadEntry,
                   CodeGen);
    break;
  case CGOpenACCRuntimeNVPTX::ExecutionMode::Unknown:
    llvm_unreachable(
        "Unknown programming model for OpenACC directive on NVPTX target.");
  }

  setPropertyExecutionMode(CGM, OutlinedFn->getName(), Mode);
}

CGOpenACCRuntimeNVPTX::CGOpenACCRuntimeNVPTX(CodeGenModule &CGM)
    : CGOpenACCRuntime(CGM), CurrentExecutionMode(ExecutionMode::Unknown) {
  if (!CGM.getLangOpts().OpenACCIsDevice)
    llvm_unreachable("OpenACC NVPTX can only handle device code.");
}

void CGOpenACCRuntimeNVPTX::emitProcBindClause(CodeGenFunction &CGF,
  ...
  CGOpenACCRuntime::emitProcBindClause(CGF, ProcBind, Loc);
}

void CGOpenACCRuntimeNVPTX::emitNumThreadsClause(CodeGenFunction &CGF,
                                                llvm::Value *NumThreads,
                                                SourceLocation Loc) {
  // Do nothing in case of Spmd mode and L0 parallel.
  // TODO: If in Spmd mode and L1 parallel emit the clause.
  ...
  CGOpenACCRuntime::emitNumThreadsClause(CGF, NumThreads, Loc);
}

void CGOpenACCRuntimeNVPTX::emitNumGangsClause(CodeGenFunction &CGF,
                                              const Expr *NumGangs,
                                              const Expr *ThreadLimit,
                                              SourceLocation Loc) {}

llvm::Value *CGOpenACCRuntimeNVPTX::emitParallelOutlinedFunction(
  ...
  auto *OutlinedFun = cast<llvm::Function>(
    CGOpenACCRuntime::emitParallelOutlinedFunction(
...
llvm::Value *CGOpenACCRuntimeNVPTX::emitGangOutlinedFunction(
    const ACCExecutableDirective &D, const VarDecl *ThreadIDVar,
    OpenACCDirectiveKind InnermostKind, const ACCRegionCodeGenTy &CodeGen) {

  llvm::Value *OutlinedFunVal = CGOpenACCRuntime::emitGangOutlinedFunction(
      D, ThreadIDVar, InnermostKind, CodeGen);
...
void CGOpenACCRuntimeNVPTX::emitGangCall(CodeGenFunction &CGF,
                                         const ACCExecutableDirective &D,
                                         SourceLocation Loc,
...
void CGOpenACCRuntimeNVPTX::emitParallelCall(
...
void CGOpenACCRuntimeNVPTX::emitGenericParallelCall(
    CodeGenFunction &CGF, SourceLocation Loc, llvm::Value *OutlinedFn,
    ArrayRef<llvm::Value *> CapturedVars, const Expr *IfCond) {
  ...
  auto &&L0ParallelGen = [this, WFn, &CapturedVars](CodeGenFunction &CGF,
    ...
    if (!CapturedVars.empty()) {
      ...
      CGF.EmitRuntimeCall(
          createNVPTXRuntimeFunction(ACCRTL_NVPTX__kmpc_kernel_prepare_parallel),
          Args);
      ...
      llvm::Value *Args[] = {
          ...
          /*nArgs=*/Bld.getInt32(0), /*RequiresACCRuntime=*/Bld.getInt16(1)};
      CGF.EmitRuntimeCall(
          createNVPTXRuntimeFunction(ACCRTL_NVPTX__kmpc_kernel_prepare_parallel),
          Args);
    }
    ...
    // OpenACC [2.5, Parallel Construct, p.49]
  ...
  auto &&SeqGen = [this, Fn, &CapturedVars, &Args, Loc](CodeGenFunction &CGF,
    ...
    NVPTXActionTy Action(
        createNVPTXRuntimeFunction(ACCRTL_NVPTX__kmpc_serialized_parallel),
        Args,
        createNVPTXRuntimeFunction(ACCRTL_NVPTX__kmpc_end_serialized_parallel),
        Args);
  ...
  if (IfCond)
    emitACCIfClause(CGF, IfCond, L0ParallelGen, SeqGen);
  else {
    CodeGenFunction::RunCleanupsScope Scope(CGF);
    ACCRegionCodeGenTy ThenRCG(L0ParallelGen);
    ThenRCG(CGF);
  }
}

void CGOpenACCRuntimeNVPTX::emitSpmdParallelCall(
    CodeGenFunction &CGF, SourceLocation Loc, llvm::Value *OutlinedFn,
    ArrayRef<llvm::Value *> CapturedVars, const Expr *IfCond) {
...
static llvm::Value *createRuntimeShuffleFunction(CodeGenFunction &CGF,
  ...
  auto &Bld = CGF.Builder;
  CGOpenACCRuntimeNVPTX &RT =
      *(static_cast<CGOpenACCRuntimeNVPTX *>(&CGM.getOpenACCRuntime()));

  unsigned Size = CGM.getDataLayout().getTypeStoreSize(Elem->getType());
  assert(Size <= 8 && "Unsupported bitwidth in shuffle instruction.");

  OpenACCRTLFunctionNVPTX ShuffleFn = Size <= 4
                                         ? ACCRTL_NVPTX__kmpc_shuffle_int32
                                         : ACCRTL_NVPTX__kmpc_shuffle_int64;
...
static llvm::Value *emitReduceScratchpadFunction(
  ...
  CGM.getOpenACCRuntime().emitOutlinedFunctionCall(
...
static llvm::Value *emitInterWarpCopyFunction(CodeGenModule &CGM,
  ...
  const char *TransferMediumName =
      "__openacc_nvptx_data_transfer_temporary_storage";
  ...
  // Get the CUDA thread id of the current OpenACC thread on the GPU.
...
/// Emit a helper that reduces data across two OpenACC threads (lanes)
...
///
/// Design of OpenACC reductions on the GPU
///
/// Consider a typical OpenACC program with one or more reduction
/// clauses:
///
/// float foo;
/// double bar;
...
/// }
///
/// where 'foo' and 'bar' are reduced across all OpenACC threads in
/// all teams.  In our OpenACC implementation on the NVPTX device an
/// OpenACC team is mapped to a CUDA threadblock and OpenACC threads
/// within a team are mapped to CUDA threads within a threadblock.
/// Our goal is to efficiently aggregate values across all OpenACC
/// threads such that:
///
///   - the compiler and runtime are logically concise, and
///   - the reduction is performed efficiently in a hierarchical
///     manner as follows: within OpenACC threads in the same warp,
...
///
/// Pseudocode for the aforementioned OpenACC program generated by the
/// compiler is as follows:
///
/// 1. Create private copies of reduction variables on each OpenACC
///    thread: 'foo_private', 'bar_private'
/// 2. Each OpenACC thread reduces the chunk of 'A' and 'B' assigned
///    to it and writes the result in 'foo_private' and 'bar_private'
///    respectively.
/// 3. Call the OpenACC runtime on the GPU to reduce within a team
...
///     of type 'ReduceData' across two OpenACC threads (lanes) in the
...
/// 4. Call the OpenACC runtime on the GPU to reduce across teams.
...
/// OpenACC runtime depending on the number of active lanes:
///
/// Full Warp Reduction
///
...
/// number of OpenACC threads in a parallel region is not a multiple of
...
/// across lanes in an OpenACC simd region or in a nested parallel region.
///
/// void
...
/// '__kmpc_nvptx_parallel_reduce_nowait', aggregates data across OpenACC
...
///
void CGOpenACCRuntimeNVPTX::emitReduction(
  ...
  bool ParallelReduction = isOpenACCParallelDirective(Options.ReductionKind);
  bool GangReduction = isOpenACCGangDirective(Options.ReductionKind);
  // FIXME: Add support for simd reduction.
  assert((GangReduction || ParallelReduction) &&
  ...
  if (ParallelReduction) {
    ...
    Res = CGF.EmitRuntimeCall(
        createNVPTXRuntimeFunction(ACCRTL_NVPTX__kmpc_parallel_reduce_nowait),
        Args);
  }
  ...
  if (GangReduction) {
    ...
    llvm::Value *Args[] = {ThreadId,
                           ...
                           LoadAndReduceFn};
    Res = CGF.EmitRuntimeCall(
        createNVPTXRuntimeFunction(ACCRTL_NVPTX__kmpc_teams_reduce_nowait),
        Args);
  }
  ...
  auto &&CodeGen = [&Privates, &LHSExprs, &RHSExprs, &ReductionOps,
                    this](CodeGenFunction &CGF, ACCPrePostActionTy &Action) {
    auto IPriv = Privates.begin();
  ...
  NVPTXActionTy Action(
      nullptr, llvm::None,
      createNVPTXRuntimeFunction(ACCRTL_NVPTX__kmpc_end_reduce_nowait),
      EndArgs);
  RCG.setAction(Action);
  RCG(CGF);
...
const VarDecl *
CGOpenACCRuntimeNVPTX::translateParameter(const FieldDecl *FD,
  ...
  if (const auto *Attr = FD->getAttr<ACCCaptureKindAttr>()) {
    if (Attr->getCaptureKind() == ACCC_map) {
...
Address
CGOpenACCRuntimeNVPTX::getParameterAddress(CodeGenFunction &CGF,
...
void CGOpenACCRuntimeNVPTX::emitOutlinedFunctionCall(
    CodeGenFunction &CGF, SourceLocation Loc, llvm::Value *OutlinedFn,
  ...
  CGOpenACCRuntime::emitOutlinedFunctionCall(CGF, Loc, OutlinedFn, TargetArgs);
}
...
llvm::Function *CGOpenACCRuntimeNVPTX::createDataSharingWrapper(
    llvm::Function *OutlinedParallelFn, const ACCExecutableDirective &D) {
  ASTContext &Ctx = CGM.getContext();
  ...
  // TODO: suppport VECTOR and pass actual values
