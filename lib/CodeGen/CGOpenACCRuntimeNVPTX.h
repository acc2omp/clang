//===----- CGOpenACCRuntimeNVPTX.h - Interface to OpenACC NVPTX Runtimes ----===//
//
...
#ifndef LLVM_CLANG_LIB_CODEGEN_CGOPENACCRUNTIMENVPTX_H
#define LLVM_CLANG_LIB_CODEGEN_CGOPENACCRUNTIMENVPTX_H
...
class CGOpenACCRuntimeNVPTX : public CGOpenACCRuntime {
private:
  ...
  /// \brief Signal termination of ACC execution for generic target entry
  /// function.
  ...
  /// evaluates to false.
  void emitGenericKernel(const ACCExecutableDirective &D, StringRef ParentName,
  ...
  /// An outlined function may not be an entry if, e.g. the if clause always
  /// evaluates to false.
  void emitSpmdKernel(const ACCExecutableDirective &D, StringRef ParentName,
                      llvm::Function *&OutlinedFn,
                      llvm::Constant *&OutlinedFnID, bool IsOffloadEntry,
                      const ACCRegionCodeGenTy &CodeGen);
...
protected:
  ...
  StringRef getOutlinedHelperName() const override {
    return "__acc_outlined__";
  }

public:
  explicit CGOpenACCRuntimeNVPTX(CodeGenModule &CGM);
  ...
  // the openacc runtime kmpc_push_num_teams. In NVPTX backend it is not needed
  ...
  /// \brief Emits inlined function for the specified OpenACC parallel
  //  directive.
  ...
  /// \brief Emits inlined function for the specified OpenACC teams
  //  directive.
  /// \a D. This outlined function has type void(*)(kmp_int32 *ThreadID,
  /// kmp_int32 BoundID, struct context_vars*).
  /// \param D OpenACC directive.
  ...
  /// Returns specified OpenACC runtime function for the current OpenACC
  ...
  /// 'generic' fork-join model of OpenACC, and a more GPU efficient 'spmd'
...
#endif // LLVM_CLANG_LIB_CODEGEN_CGOPENACCRUNTIMENVPTX_H
