...
void tools::AddLinkerInputs(const ToolChain &TC, const InputInfoList &Inputs,
  ...
  for (const auto &II : Inputs) {
    // If the current tool chain refers to an OpenACC offloading host, we should
    // ignore inputs that refer to OpenACC offloading devices - they will be
    // embedded according to a proper linker script.
    if (auto *IA = II.getAction())
      if (JA.isHostOffloading(Action::OFK_OpenACC) &&
          IA->isDeviceOffloading(Action::OFK_OpenACC))
        continue;
...
bool tools::addOpenACCRuntime(ArgStringList &CmdArgs, const ToolChain &TC,
...
/// Add OpenACC linker script arguments at the end of the argument list so that
/// the fat binary is built by embedding each of the device images into the
/// host. The linker script also defines a few symbols required by the code
/// generation so that the images can be easily retrieved at runtime by the
/// offloading library. This should be used only in tool chains that support
/// linker scripts.
void tools::AddOpenACCLinkerScript(const ToolChain &TC, Compilation &C,
                                  const InputInfo &Output,
                                  const InputInfoList &Inputs,
                                  const ArgList &Args, ArgStringList &CmdArgs,
                                  const JobAction &JA) {

  // If this is not an OpenACC host toolchain, we don't need to do anything.
  if (!JA.isHostOffloading(Action::OFK_OpenACC))
    return;
  ...
  // Get the OpenACC offload tool chains so that we can extract the triple
  // associated with each device input.
  auto OpenACCToolChains = C.getOffloadToolChains<Action::OFK_OpenACC>();
  assert(OpenACCToolChains.first != OpenACCToolChains.second &&
         "No OpenACC toolchains??");
  ...
  // machines.
  LksStream << "/*\n";
  LksStream << "       OpenACC Offload Linker Script\n";
  LksStream << " *** Automatically generated by Clang ***\n";
  LksStream << "*/\n";
  LksStream << "TARGET(binary)\n";
  auto DTC = OpenACCToolChains.first;
  for (auto &II : Inputs) {
    const Action *A = II.getAction();
    // Is this a device linking action?
    if (A && isa<LinkJobAction>(A) &&
        A->isDeviceOffloading(Action::OFK_OpenACC)) {
      assert(DTC != OpenACCToolChains.second &&
  ...
  if (C.getArgs().hasArg(options::OPT_fopenacc_dump_offload_linker_script))
    llvm::errs() << LksBuffer;
